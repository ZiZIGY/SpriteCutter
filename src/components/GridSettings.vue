...
<template>
  ...
  <VMenu v-model="colorPickerOpen" :close-on-content-click="false" location="right">
    <template #activator="{ props: menuProps }">
      ...
      >
        <span
          class="mr-2 rounded"
          :style="{ background: store.gridColor, width: '16px', height: '16px', flexShrink: 0, border: '1px solid rgba(255,255,255,0.2)', display: 'inline-block' }"
        />
        {{ store.gridColor.toUpperCase() }}
        <VIcon end>mdi-menu-down</VIcon>
      </VBtn>
    </template>
    <VCard rounded="lg" elevation="8" style="overflow: hidden">
      <VColorPicker
      ...
      />
      <VCardActions>
        <VBtn size="x-small" variant="text" @click="store.gridColor = '#FFFFFF'; colorPickerOpen = false">
          <VIcon start size="13">mdi-restore</VIcon>По умолчанию
        </VBtn>
        <VSpacer />
        <VBtn size="x-small" variant="tonal" @click="colorPickerOpen = false">
          Готово
        </VBtn>
      </VCardActions>
    </VCard>
...
<style scoped>
.fade-section-enter-active, .fade-section-leave-active { transition: opacity 0.15s ease; }
.fade-section-enter-from, .fade-section-leave-to { opacity: 0; }
</style>
